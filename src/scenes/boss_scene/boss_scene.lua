...
local lightDefs = {
    {x=120, y=300, size=800},
    {x=500, y=288, size=1000}
}
...
function boss:init()
    ...
    error([[todo; fix resize bug; canvas isnt resized]])

    self.lightWorld = newLightWorld()
    for _, def in ipairs(lightDefs) do
        self.lightWorld:newLight(def.x, def.y, def.size)
    end
end
